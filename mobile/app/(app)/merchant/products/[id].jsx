...
export default function ProductEditScreen() {
  ...
  useEffect(() => {
    ...
    productService.getProduct(String(id)).then((doc) => {
      setName(doc.name || '')
      setPrice(String(doc.price ?? ''))
      setStock(String(doc.stock ?? '0'))
      setCategory(doc.category || '')
      setAvailable(Boolean(doc.available))
    }).catch(() => {})
  }, [id, user])
  ...
  async function save() {
    ...
    try {
      ...
      if (imageUri) {
        ...
        payload.image_id = upload.$id
      }

      await productService.updateProduct(String(id), payload)
    ...
    <ScrollView style={styles.screen} contentContainerStyle={styles.content}>
      ...
      <View style={styles.card}>
        ...
        <Pressable style={styles.secondaryBtn} onPress={pickImage}>
          <Text style={styles.secondaryBtnText}>{imageUri ? 'Image selected' : 'Replace image'}</Text>
        </Pressable>
